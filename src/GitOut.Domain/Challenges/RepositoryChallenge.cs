...
/// <summary>
/// A challenge that validates git repository state
/// </summary>
public class RepositoryChallenge : IChallenge
{
    ...
    public ChallengeType Type => ChallengeType.Repository;
    ...
    public List<string> RequiredStagedFiles { get; init; } = new();
    public List<string> SetupFiles { get; init; } = new();

    public RepositoryChallenge(
        string id,
        string description,
        ...
        List<string>? requiredStagedFiles = null,
        ...
        Func<string, IGitCommandExecutor, Task>? customSetup = null)
    {
        ...
        RequiredStagedFiles = requiredStagedFiles ?? new List<string>();
    ...
    public async Task<ChallengeResult> ValidateAsync(string workingDirectory)
    {
        ...
        foreach (var fileName in RequiredFiles)
        {
            ...
            if (!File.Exists(filePath))
            {
                return new ChallengeResult(
                    ...
                    $"Make sure the file '{fileName}' exists in the working directory."
                );
            }
        }
        ...
        // Check staged files
        if (RequiredStagedFiles.Count > 0)
        {
            var status = await _gitExecutor.GetStatusAsync(workingDirectory);

            foreach (var fileName in RequiredStagedFiles)
            {
                if (!status.Contains($"new file:   {fileName}") &&
                    !status.Contains($"modified:   {fileName}"))
                {
                    return new ChallengeResult(
                        false,
                        $"File '{fileName}' is not staged.",
                        $"Try running 'git add {fileName}' to stage the file."
                    );
                }
            }
        }
